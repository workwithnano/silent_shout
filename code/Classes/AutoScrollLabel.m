...
#define DEFAULT_PIXELS_PER_SECOND 90
#define DEFAULT_PAUSE_TIME 0.0f
...
- (void)animationDidStop:(NSString *)animationID finished:(NSNumber *)finished context:(void *)context
{
  ...
  //NSLog(@"animationDidStop fired with 0");
...
- (void)animationDidStop:(NSString *)animationID finished:(NSNumber *)finished context:(void *)context
{
  ...
  //NSLog([NSString stringWithFormat:@"%@%d", @"[finished intValue] = ", [finished intValue]]);
  //NSLog([NSString stringWithFormat:@"%@%f", @"AutoScrollLabel->label[0].frame.size.width = ", label[0].frame.size.width]);
  //NSLog([NSString stringWithFormat:@"%@%f", @"AutoScrollLabel->self.frame.size.width = ", self.frame.size.width]);
  
	if (label[0].frame.size.width > self.frame.size.width){
    //NSLog(@"Setting the timer to inch the label across the screen");
...
- (void) scroll
{
	...
	if (isScrolling){
    NSLog(@"isScrolling = YES");
    return;
	}
  else {
    NSLog(@"isScrolling = NO");
  }
	isScrolling = YES;
	...
	NSLog(@"About to beginAnimations:@'scroll'");
...
- (void) readjustLabels
{
  ...
  NSLog(@"Reajusting Labels");
    ...
    NSLog(@"Label is not bigger than space allocated. Will not scroll.");
...
{	
  // If not dragging, send event of a tap
  if (!self.dragging) 
